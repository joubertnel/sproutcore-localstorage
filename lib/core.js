...
SC.LocalStorage = SC.Object.extend(/** @scope SC.LocalStorage.prototype */ {
  ...
  */
  ...
  /**
     Invoke this method to set the built-in defaults. Observers are notified
     that each of the new defaults have changed.
  */
  defaults: function(newDefaults) {
    this._defaults = newDefaults ;

    // Notify observers that default values have changed
  ...
  */
  readValue: function(keyName) {
    ...
    if ((ret === undefined) && this._defaults) {
      ret = this._defaults[defaultKeyName];
    }
  ...
  */
  ...
  /**
     Proxies a .get() to underlying localStorage so that we participate
     in bindings.
  ...
  */
  ...
  /**
     Proxies a .set() to underlying localStorage so that we participte
     in bindings.
  ...
  */
  ...
  _domainDidChange: function() {
    ...
    if (didChange) this._allKeyValPropertiesDidChange();
  }.observes('userDomain', 'appDomain'),


  _allKeyValPropertiesDidChange: function() {
    // TODO: notify the platform that all properties have changed
  },
